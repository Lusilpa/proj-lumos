...
export default function SetoresAdminPage() {
  ...
  const handleCreateSetor = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newSetorId = await setorService.createSetor({ nome: formData.nome, slug });
      const newSetor: Setor = { id: newSetorId, nome: formData.nome, slug };
      setLocalSetores(prev => [...prev, newSetor]);
  ...
  const handleEditSetor = async (id: string, nome: string) => {
    try {
      const slug = nome.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '');
      await setorService.updateSetor(id, { nome, slug });
      setLocalSetores(prev => prev.map(s => s.id === id ? { ...s, nome, slug } : s));
  ...
  const handleDeleteSetor = async (id: string) => {
    try {
      ...
      setLocalSetores(prev => prev.filter(s => s.id !== id));
  ...
  const setoresExibidos = localSetores.filter((setor) => {
    ...
    return setor.nome.toLowerCase().includes(termo) || setor.slug.toLowerCase().includes(termo);
  });
  ...
  return (
    <div className="max-w-6xl mx-auto flex flex-col h-full relative">
      <div className="flex flex-col gap-2 mb-8">
        ...
        <p className="text-brand-300 font-medium">Cadastro e estruturação de departamentos organizacionais do laboratório.</p>
      </div>

      <ToolsSetores searchQuery={searchQuery} setSearchQuery={setSearchQuery} setIsModalOpen={setIsModalOpen} />

      {isLoading ? (
        <div className="flex justify-center p-8"><span className="text-brand-300">Carregando...</span></div>
      ) : (
        <TableSetores setoresExibidos={setoresExibidos} onEdit={handleEditSetor} onDelete={handleDeleteSetor} />
      )}

      <CadastroGeralSetoresPage
        isModalOpen={isModalOpen} setIsModalOpen={setIsModalOpen}
        formData={formData} setFormData={setFormData}
        handleCreateSetor={handleCreateSetor} isSubmitting={isSubmitting}
      />
    </div>
  );
}
